...
# Optional: set NVIDIA_MODEL_NAME to pin the exact model id for your endpoint.
astro_pilot.get_idea(mode="fast",llm='nemotron-nano') 

# This module checks if the idea is novel or not against previous literature
astro_pilot.check_idea(llm='nemotron-nano', max_iterations=7) 

# This module generates the methodology to be employed.
astro_pilot.get_method(mode="fast",llm='nemotron-nano') 

# This module writes codes, executes the codes, makes plots,
#  and summarizes the results.
astro_pilot.get_results(engineer_model='nemotron-nano',
                        researcher_model='nemotron-nano',
                        planner_model='nemotron-nano',
                        plan_reviewer_model='nemotron-nano',
                        orchestration_model='nemotron-nano',
                        formatter_model='nemotron-nano',
                        )

# Get the paper
astro_pilot.get_paper(journal=Journal.AAS, llm='nemotron-nano', add_citations=False) 

# Referee the paper
astro_pilot.referee(llm='nemotron-nano')
